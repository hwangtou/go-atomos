...
func (p *CosmosProcess) trySettingClusterToCurrentAndKeepalive() *Error {
	...
	go func(firstLease *clientv3.LeaseGrantResponse) {
		...
		defer func() {
			p.local.Log().coreInfo("etcd: Watcher will stop.")
		...
		for {
			...
			case keepAlive := <-keepAliveCh:
				...
				if keepAlive == nil {
					p.local.Log().coreInfo("etcd: Watcher keepalive stopped, lease is not renewed.")
					return
				}
			// 更新节点信息
			// Update node information
			case infoBuf, more := <-p.cluster.etcdInfoCh:
				p.local.Log().coreInfo("etcd: Watcher updates process info. info=(%s),more=(%v)", infoBuf, more)
				if !more {
					return
				}
		...
		for {
			...
			for watchResp := range watchCh {
				for _, event := range watchResp.Events {
					p.local.Log().coreInfo("etcd: Watch event. type=(%s),key=(%s),value=(%s)", event.Type, string(event.Kv.Key), string(event.Kv.Value))
...
func (p *CosmosProcess) tryUnsettingCurrentAndUpdateNodeInfo() *Error {
	...
	if err := p.etcdStoppingNodeVersionUnlock(); err != nil {
		p.local.Log().coreFatal("etcd: Failed to unset cluster current. err=(%v)", err)
	}
	// 更新节点信息
	// Update node information
...
func (p *CosmosProcess) etcdUpdateClusterVersionLockInfo(nodeName string, buf []byte) {
	p.local.Log().coreInfo("etcd: Update current version lock. node=(%s),lock=(%s)", nodeName, string(buf))
...
func (p *CosmosProcess) etcdDeleteClusterVersionLockInfo(nodeName string) {
	p.local.Log().coreInfo("etcd: Delete current version lock. node=(%s)", nodeName)
	p.cluster.remoteMutex.Lock()
	defer p.cluster.remoteMutex.Unlock()
...
func (p *CosmosProcess) etcdUpdateClusterVersionNodeInfo(nodeName string, versionKey string, value []byte) {
	p.local.Log().coreInfo("etcd: Update cluster version info. node=(%s),version=(%s)", nodeName, versionKey)
...
func (p *CosmosProcess) etcdDeleteClusterVersionNodeInfo(nodeName, versionKey string) {
	p.local.Log().coreInfo("etcd: Delete cluster version info. node=(%s),version=(%s)", nodeName, versionKey)
	p.cluster.remoteMutex.Lock()
	defer p.cluster.remoteMutex.Unlock()
...
func (p *CosmosProcess) etcdErrorHandler(err *Error) {
	p.local.Log().Fatal("etcd: CRITICAL ERROR. err=(%s)", err)
}
