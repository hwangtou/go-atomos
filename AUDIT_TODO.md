...
| E9 | `grpc.DialContext` deprecated（grpc v1.79） | cosmos_remote.go、cosmos_process_test.go | ✅ 已修复 | check() 迁移 `grpc.NewClient`；NewClient 懒连接且忽略 WithBlock，用 `Connect()` + `WaitForStateChange`（1s 超时，含 TLS 握手）保留原阻塞拨号 fail-fast 语义——对端宕机仍在 check() 内 ~1s 报错而非挂到各 RPC 自身 deadline；失败路径保留 net.DialTimeout 诊断日志；测试 fixture 同步迁移 |
| E10 | 仓库卫生：两个 protoc 插件编译产物（8.0M + 9.6M）被提交进版本控制；根目录有空的 `~` 目录 | protoc-gen-go-atomos/、.gitignore | ✅ 已修复 | `git rm --cached` 摘除两个二进制（本地文件保留，build.sh 可随时重建）；.gitignore 补两条二进制规则 + `/~` 防御规则；空 `~` 目录已删除。注意：历史快照中仍含二进制，如需瘦身历史须 filter-repo（未做） |
