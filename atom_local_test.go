...
func TestAtomLocalBase(t *testing.T) {
	...
	// Spawn an atom.
	atom, tracker, err := spawnAtomLocal(testElem, testAtomName, &String{S: testAtomName})
	if err != nil {
	...
	// Spawn an atom.
	deadlockAtom, deadlockTracker, err := spawnAtomLocal(testElem, testDeadlockAtomName, &String{S: testDeadlockAtomName})
	if err != nil {
	...
	// Try Spawning twice.
	atomTwice, trackerTwice, err := spawnAtomLocal(testElem, testAtomName, &String{S: testAtomName})
	...
	spawnPanicAtom, trackerSpawnPanicAtom, err := spawnAtomLocal(testElem, spawnPanicAtomName, &String{S: "panic"})
	if err == nil || len(err.CallStacks) == 0 || err.CallStacks[0].PanicStack == "" {
	...
	stoppingPanicAtom, trackerStoppingPanicAtom, err := spawnAtomLocal(testElem, stoppingPanicAtomName, nil)
	if err != nil {
	...
	dev := runnable.implements["testElement"].Developer.(*testElementDev)
	dev.autoData = true

	// GetAtomData returns Error.
	dataAtomName := "get_data_error"
	dataAtom, trackerDataAtom, err := spawnAtomLocal(testElem, dataAtomName, nil)
	if err == nil {
	...
	inner := dataAtom.atomos.instance.(*testAtomos)
...
func spawnAtomLocal(elem *ElementLocal, name string, arg proto.Message) (*AtomLocal, *IDTracker, *Error) {
	return elem.SpawnAtom(name, arg, 2)
}
...
func checkAtomLocalInElement(t *testing.T, elem *ElementLocal, name string, isNotExist bool, isState AtomosState, refCount int) *Error {
	...
	case AtomosWaiting:
		if !atom.atomos.IsInState(AtomosWaiting) {
			return NewError(ErrFrameworkPanic, "Atom should be waiting")
		}
	case AtomosBusy:
		if !atom.atomos.IsInState(AtomosBusy) {
			return NewError(ErrFrameworkPanic, "Atom should be busy")
		}
	case AtomosStopping:
		if !atom.atomos.IsInState(AtomosStopping) {
			return NewError(ErrFrameworkPanic, "Atom should be stopping")
		}
	}
