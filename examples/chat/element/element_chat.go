...
func (c *ChatRoomElement) Check() error {
	return nil
}

func (c *ChatRoomElement) Loaded(mainId atomos.Id) {
	c.mainId = mainId
}

func (c *ChatRoomElement) Unloaded() {
	c.mainId = nil
}
...
func (c *chatRoomAtom) SendMessage(from atomos.Id, in *api.SendMessageReq) (*api.SendMessageResp, error) {
	...
	taskId, err := c.self.Task().Append(c.sendMessage, in)
	if err != nil {
