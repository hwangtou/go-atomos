...
func main() {
	...
	runnable.AddElementImplementation(api.GetChatManagerAtomImplement())
	runnable.AddElementImplementation(api.GetChatAtomImplement())
...
func scriptChat(cosmos *atomos.CosmosSelf, mainId atomos.MainId, killNoticeChannel chan bool) {
	// Spawn UserManager
	userManagerId, err := api.SpawnUserManagerAtom(cosmos.Local(), element.UserManagerElementName, &api.UserManager{})
	if err != nil {
	...
	// Spawn ChatManager
	chatManagerId, err := api.SpawnUserManagerAtom(cosmos.Local(), element.UserManagerElementName, &api.UserManager{})
	if err != nil {
