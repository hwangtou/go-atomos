...
func main() {
	cosmosName := "hello_cosmos"
	nodeName := "cosmos1"
	watcher, err := go_atomos.NewNodeWatcher(cosmosName, nodeName)
	if err != nil {
		log.Fatalln(err)
	}
	defer watcher.Close()

	if watcher.GetSupervisorConfig().IsKeepaliveNode(nodeName) {
		running, err := watcher.IsRunning()
		if err != nil {
			log.Println("Keepalive failed", err)
		}
		if !running {
			if err := watcher.LaunchProcess(); err != nil {
				log.Println("Launch failed", err)
			}
		}
	}

	isRunning, err := watcher.IsRunning()
	if err != nil {
		log.Println(err)
	}
	log.Println("Running", isRunning)

	for {
		changed := <-watcher.OnChanged()
		switch changed {
		case go_atomos.WatcherHalt:
			log.Println("Halt")
			if watcher.GetSupervisorConfig().IsKeepaliveNode(nodeName) {
